...
# Chat with a character through a local SAM API running with USE_MOCK_LLM=true.
# Run examples/curl/create-character-local.sh first, then export the returned ID:
...
: "${CHARACTER_ID:?Set CHARACTER_ID to a character_id returned by create-character-local.sh}"

BASE_URL="${LOCAL_API_BASE_URL:-http://127.0.0.1:3000}"
SESSION_ID="${SESSION_ID:-session-demo-local-1}"
PLAYER_ID="${PLAYER_ID:-player-demo-1}"
PLAYER_MESSAGE="${PLAYER_MESSAGE:-I can help recover the sky map. What needs doing first?}"
RESPONSE_FILE="${RESPONSE_FILE:-/tmp/characterforge-chat-local.json}"
REQUEST_FILE="$(mktemp)"
