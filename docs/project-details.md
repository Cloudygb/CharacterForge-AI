...
CharacterForge AI turns that pattern into a small platform: author characters locally, test them through the dashboard, package reusable content, and deploy the runtime to AWS when you are ready.
...
- Document the HTTP API with [`openapi.yaml`](../openapi.yaml) and curl examples.
...
The repository includes ready-to-run scripts under [`examples/curl/`](../examples/curl/):
...
Do not commit real API key values. Keep them in your shell environment, local secret manager, or deployment-specific configuration.
...
- For dashboard work: Node.js, npm, Rust, and the Tauri prerequisites for your OS
...
The staged Windows installer path is:
...
├── docs/
│   ├── assets/                  # README screenshots and architecture image
...
│   └── game-binding.schema.json
├── scripts/
│   ├── bedrock_smoke_test.py
│   ├── character_pack_export.py
│   └── character_pack_import.py
├── sdk/
