...
| CF-AUDIT-1.2 | 1.2 Broken object-level authorization risk | P0 | In progress | Step 15 model commit recorded in final task summary; Step 16 auth-regression commit recorded in final task summary; Step 19 ownership metadata commit pending | `pytest tests/test_identity_model_documentation.py -q`; `pytest tests/test_authorization.py -q`; `pytest tests/test_character_store.py tests/test_session_store.py -q`; `pytest tests/test_best_practice_audit_regressions.py -q` | Production identity model defines tenant/game/environment ownership dimensions and route-level ownership rules; Step 16 adds xfailed regression tests for cross-tenant character/session reads and service-token game/environment scope; Step 19 stores tenant_id, game_id, environment_id, created_by, updated_by, and audit timestamps on character/session records with explicit legacy dev data backfill. Enforcement remains in later implementation steps. |
...
| Verification summary | `pytest tests/test_authorization.py -q` returned 4 xfailed after Step 18 made unauthenticated and malformed-authorizer production requests return 401; central audit regression tests, ledger tests, and full Python suite pass with remaining scope/object xfails visible. Final verification commands are recorded in the Step 18 task summary. |
