...
## Step 22 Production CORS Restriction

| Evidence Item | Result |
| --- | --- |
| Audit coverage | CF-AUDIT-1.4 / P0/P1 wildcard CORS is too permissive. |
| RED tests | `pytest tests/test_infra_security.py tests/test_best_practice_audit_regressions.py::test_no_wildcard_production_cors tests/test_audit_remediation_ledger.py::test_step_22_evidence_records_production_cors_restriction -q` failed before implementation because production CORS still used a wildcard origin, `AllowedCorsOrigins` and `DevMode` did not exist, and `Authorization` was advertised in allowed headers. |
| Origin parameterization | `infra/template.yaml` exposes `AllowedCorsOrigins`; the production default is no browser origin, and deployed browser access requires explicit configured origins. |
| Dev-only wildcard | Wildcard CORS is available only through `UseDevCorsWildcard`, which depends on the explicit `DevMode` parameter labeled for local development only and not for production. |
| Header restriction | CORS allowed headers are `Content-Type,x-api-key`; `Authorization` was removed from advertised CORS headers until an intentional browser bearer-token flow is designed and approved. |
