...
| CF-AUDIT-4.3 | 4.3 Prerequisite installers need stronger pinning and failure handling | P1 | Implemented | Step 10 commit recorded in final task summary | `pytest tests/test_installer_prerequisite_installers.py -q`; `pytest tests/test_best_practice_audit_regressions.py -q`; `pytest tests/test_audit_remediation_ledger.py -q` | NSIS now pops and checks every prerequisite helper return code, reports actionable summaries, fails required helper failures, and treats optional Docker guidance failures as warnings. |
| CF-AUDIT-4.4 | 4.4 Installer scope, downgrade, and MSI/NSIS strategy need cleanup | P1 | Implemented | Step 9 commit recorded in final task summary | `pytest tests/test_dashboard_tauri_packaging.py::test_tauri_release_installers_block_normal_downgrades -q`; `pytest tests/test_best_practice_audit_regressions.py -q`; installer build/config inspection | Tauri Windows bundle config explicitly sets `allowDowngrades: false`, covering normal MSI/NSIS release installer flows so older installers cannot replace newer installs by default. |
