...
| --- | --- | --- | --- | --- | --- | --- |
| CF-AUDIT-1.1 | 1.1 API key only is not sufficient authentication | P0 | In progress | Step 15 model commit recorded in final task summary; Step 16 auth-regression commit recorded in final task summary; Step 17 authorizer commit pending | `pytest tests/test_identity_model_documentation.py -q`; `pytest tests/test_authorization.py -q`; `pytest tests/test_api_authorizer_infrastructure.py -q`; `pytest tests/test_openapi_api_key_security.py -q`; `pytest tests/test_best_practice_audit_regressions.py -q` | Production identity model defines human and service principals, token types, scopes, and states that API keys are metering only; Step 16 adds xfailed regression tests for unauthenticated requests, invalid bearer tokens, and read-only token write attempts; Step 17 adds configurable Cognito/OIDC JWT authorizer infrastructure, a local development only mode, and BearerAuth OpenAPI documentation. Handler enforcement remains in later auth implementation steps. |
| CF-AUDIT-1.2 | 1.2 Broken object-level authorization risk | P0 | In progress | Step 15 model commit recorded in final task summary; Step 16 auth-regression commit pending | `pytest tests/test_identity_model_documentation.py -q`; `pytest tests/test_authorization.py -q`; `pytest tests/test_best_practice_audit_regressions.py -q` | Production identity model defines tenant/game/environment ownership dimensions and route-level ownership rules; Step 16 adds xfailed regression tests for cross-tenant character/session reads and service-token game/environment scope. Enforcement remains in later implementation steps. |
...
| RED tests | `pytest tests/test_authorization.py -q` reports 6 xfailed tests against the current API-key-only/global-object implementation, making the missing auth enforcement visible before handler changes. |
...
| Verification summary | `pytest tests/test_authorization.py -q` returned 6 xfailed; central audit regression tests, ledger tests, and full Python suite pass with those xfails visible. Final verification commands are recorded in the Step 16 task summary. |
